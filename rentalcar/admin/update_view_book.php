...
<div class="container">
  <br>
  <h3 style="color: white">PROCESSING BOOK</h3>
	<?php
		$query = "SELECT * FROM book  WHERE  status='processing' GROUP BY book_id";
   ...
   <tr align="left">
    ...
    <table width="100%" border="1" align="center" cellpadding="0" cellspacing="0">
      <tr align="left">
		<th width="3%">No</td>
        <th width="23%">Booking ID</th>       
        <th width="17%">User ID</th>
        <th width="25%">Booking Date</th>
        <th width="9%">Visit Date</th>
	  ...
	  ?>
        <td>&nbsp;<?php echo $a+1; ?></td>
        <td>&nbsp;<?php echo ucwords (strtolower($row['book_id'])); ?></td>
		<td>&nbsp;<?php echo ucwords (strtolower($row['user_id'])); ?></td>  
		<td>&nbsp;<?php echo ucwords (strtolower($row['book_date'])); ?></td>  
		<td>&nbsp;<?php echo ucwords (strtolower($row['visit_date'])); ?></td>
        <td width="5%" align="center"><a class="one" href="detail_book.php?id=<?php echo $row['book_id'];?>"><button class = "button1">Detail</button></a></td>
		<td width="5%" align="center"><a class="one" href="update_book.php?id=<?php echo $row['book_id'];?>"><button class = "button1">Update</button></a></td>
       </tr> 
      <?php 
       $color="2";}
	   else{
...
<br>
  <br>
  <br>
  <h3 style="color: white">COMPLETED BOOKING</h3>
	<?php
		$query = "SELECT * FROM book b, book_detail bd  WHERE b.book_id = bd.book_id AND b.status='complete' GROUP BY b.book_id";
		$result = mysqli_query($dbconn, $query) or die ("Error: " . mysqli_error($dbconn));
		$numrow = mysqli_num_rows($result);
	?>
   <tr align="left">
